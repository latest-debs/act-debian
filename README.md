![act for Debian](.github/readme-header.png)

# act for Debian

[nektos/act](https://github.com/nektos/act) — Runs GitHub Actions workflows locally in Docker containers, letting developers test CI changes without pushing to GitHub. —
...
Or download a `.deb` from the [Releases](https://github.com/latest-debs/act-debian/releases) page:

```sh
sudo dpkg -i act_*.deb
```
...
- amd64, arm64, armhf, i386, ppc64el, riscv64, s390x — whichever
  architectures nektos/act actually publishes a Linux binary for
...
For issues with act itself, see
